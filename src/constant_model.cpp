...
renderer::constant_model::constant_model(const std::string& mesh_, const glm::mat4& initialMatrix) : model(mesh_, initialMatrix) {}

void renderer::constant_model::draw()
{
	spConstant->use();//Aktywacja programu cieniuj¹cego
	glUniformMatrix4fv(spConstant->u("P"), 1, false, glm::value_ptr(renderer::P));
	glUniformMatrix4fv(spConstant->u("V"), 1, false, glm::value_ptr(renderer::V));
